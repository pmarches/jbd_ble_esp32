...
static const char *TAG = "NETWORK_LOGGING";

#define LOGGING_LISTEN_PORT 85
...
// #define CONFIG_EXAMPLE_RESET_PROVISIONED
...
void perform_provisioning(void)
{
    /* Initialize TCP/IP */
...
int sendFully(int socket, uint8_t* bytes, size_t nbBytes){
    if(bytes==NULL || nbBytes==0){
        return 0;
    }
    while(nbBytes>0){
        ssize_t nbBytesSent=send(socket, bytes, nbBytes, 0);
        if(nbBytesSent<0){
            return nbBytesSent;
        }
        nbBytes=nbBytes-nbBytesSent;
        bytes=bytes+nbBytesSent;
    }
    return 0;
}
...
static void tcp_server_task(void *pvParameters)
{
    ...
    if (addr_family == AF_INET) {
        ...
        dest_addr_ip4->sin_port = htons(LOGGING_LISTEN_PORT);
    ...
    ESP_LOGI(TAG, "Socket bound, port %d", LOGGING_LISTEN_PORT);
