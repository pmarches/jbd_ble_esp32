...
esp_bt_uuid_t JBD_WRITABLE_CHAR_UUID = { //0000ff02-0000-1000-8000-00805f9b34fb
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        ESP_LOGI(GATTC_TAG, "uuidlen=%d uuid16=%x", p_data->search_res.srvc_id.uuid.len, p_data->search_res.srvc_id.uuid.uuid.uuid16);
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ...
        esp_gatt_status_t ret_status = esp_ble_gattc_read_char(gattc_if,gl_profile_tab[PROFILE_A_APP_ID].conn_id,gl_profile_tab[PROFILE_A_APP_ID].char_handle,ESP_GATT_AUTH_REQ_NONE);
#if 0
        {
            uint16_t count = 0;
    ...
    case ESP_GATTC_READ_CHAR_EVT:        
        ESP_LOGI(GATTC_TAG, "ESP_GATTC_READ_CHAR_EVT value len=%d", param->read.value_len);
...
//             ESP_LOGI(GATTC_TAG, "searched Adv Data Len %d, Scan Response Len %d", scan_result->scan_rst.adv_data_len, scan_result->scan_rst.scan_rsp_len);
            ...
            uint8_t* service_uuid = esp_ble_resolve_adv_data(scan_result->scan_rst.ble_adv, ESP_BLE_AD_TYPE_16SRV_PART, &service_uuid_len);
            if(service_uuid_len==0){
                break;
            }
            esp_log_buffer_hex(GATTC_TAG, service_uuid, service_uuid_len);
            if(0xFF==service_uuid[0] && 0x00==service_uuid[1]){
                ESP_LOGW(GATTC_TAG, "Found service");
            }
            ...
            ESP_LOGW(GATTC_TAG, "Found manufacturer flags");
            esp_log_buffer_hex(GATTC_TAG, manufacturer_bytes, manufacturer_bytes_len);
            if(connect == false) {
                connect = true;
                ESP_LOGI(GATTC_TAG, "connect to %.*s", adv_name_len, adv_name);
        ...
        case ESP_GAP_SEARCH_INQ_CMPL_EVT:
            ESP_LOGI(GATTC_TAG, "Search complete");
            break;
        default:
            break;
        }
